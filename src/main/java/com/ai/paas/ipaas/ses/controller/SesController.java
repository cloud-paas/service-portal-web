...
public class SesController {
	private static final Logger LOGGER = LogManager.getLogger(SesController.class.getName());
	@Reference
	private ISysParamDubbo iSysParam;
	
	@RequestMapping(value="/SesApplyPage",method={RequestMethod.GET})
	public String SesApplyPage(HttpServletRequest request,HttpServletResponse response)
	{
	...
	public String mapping(ModelMap model,HttpServletRequest request,HttpServletResponse response) {
	...
	public String saveMapping(ModelMap model,HttpServletRequest request,HttpServletResponse response) {
		String serviceId =  request.getParameter("serviceId");
		...
		String a  = "";
		try {
			   a  = HttpClientUtil.sendPostRequest("http://10.1.228.198:10888/ipaas/ses/manage/mapping",
					json);
	...
	@RequestMapping(value="/sesApplyPage",method={RequestMethod.GET})
	public  String sesApplyPage(ModelMap model,HttpServletRequest request,HttpServletResponse response)
	{
        String[] clusterNumList=CacheUtils.getOptionByKey("SES.clusterNum", "param").split(",");
        String[] shardNumList=CacheUtils.getOptionByKey("SES.shardNum", "param").split(",");
        String[] sesMemList=CacheUtils.getOptionByKey("SES.sesMem","param").split(",");
        String[] replicasNumList=CacheUtils.getOptionByKey("SES.replicasNum","param").split(",");
	...
	@RequestMapping(value="sesApply",method={RequestMethod.POST},produces="application/json;charset=utf-8")
	
	public @ResponseBody String sesApply(HttpServletRequest request,HttpServletResponse response) throws IOException, URISyntaxException, PaasException
	{
	
		OrderDetailRequest orderDetailRequest=new OrderDetailRequest();
		orderDetailRequest.setProdId(Constants.serviceType.SES_CENTER);
		orderDetailRequest.setProdType(Constants.ProductType.IPAAS_JiSuan); // PROD_IAAS // 1存储  2计算   3 数据库服务
