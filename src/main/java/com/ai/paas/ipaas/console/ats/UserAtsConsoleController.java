...
public class UserAtsConsoleController {
	...
	public Map<String, Object> queryAtsList(HttpServletRequest req,
		...
		try {
			...
			String prodId = String
					.valueOf(Constants.serviceType.TRANSACTION_CENTER);
			vo.setUserServiceId(prodId);
			selectWithNoPageRequest.setSelectRequestVo(vo);
			response = atsConsoleDubboSv
	...
	public String toModifyAtsServPwd(HttpServletRequest req,
			HttpServletResponse resp) {
		SelectWithNoPageResponse<UserProdInstVo> response = null;
		...
		try {
			...
			if (Constants.OPERATE_CODE_SUCCESS.equals(response
					.getResponseHeader().getResultCode())) {
				req.setAttribute("userProdInstVo", response.getResultList()
						.get(0));
				HttpSession session = req.getSession();
				long timeStamp = System.currentTimeMillis();
				session.setAttribute(String.valueOf(timeStamp), response
						.getResultList().get(0));
	...
	public String searchUsages(HttpServletRequest request,
			HttpServletResponse response) throws PaasException, IOException,
			URISyntaxException {
		...
		userProdInstVo.setUserServiceId(String
				.valueOf(Constants.serviceType.TRANSACTION_CENTER));
		String error = "";
		...
		String result = HttpClientUtil.sendPostRequest(iPaasDubboUrl+ "/ats/console/searchUsages", params);
		Gson gson = new Gson();
		SelectWithNoPageResponse<UserProdInstVo> resultresponse = gson
				.fromJson(
						result,
						new TypeToken<SelectWithNoPageResponse<UserProdInstVo>>() {
						}.getType());
		if (resultresponse.getResponseHeader().getResultCode()
				.equals(Constants.OPERATE_CODE_FAIL)) {
			request.setAttribute("resultCode", resultresponse
					.getResponseHeader().getResultCode());
			request.setAttribute("resultMessage", resultresponse
					.getResponseHeader().getResultMessage());
			request.setAttribute("userServId", Long.parseLong(userServId));
			return "console/ats/atsDetail";
		}
		request.setAttribute("userServId", resultresponse.getResultList()
				.get(0).getUserServId());
		request.setAttribute("resultCode", resultresponse.getResponseHeader()
				.getResultCode());
		...
		request.setAttribute("signatureId", prodInstVo.getAtsUserPageVo()
				.getTopicUsage().get(0).getTopicEnName());
		request.setAttribute("partitions", prodInstVo.getAtsUserPageVo()
				.getTopicUsage().size());
	...
	public @ResponseBody Map<String, Object> searchOneMessage(
		...
		params.put("serviceId",
				String.valueOf(Constants.serviceType.TRANSACTION_CENTER));
		String errorinfo = "";
		...
		String result = HttpClientUtil.sendPostRequest(iPaasDubboUrl + "/ats/console/searchOneMessage", data);

		JSONObject jsonObject = new JSONObject(result);
		if (jsonObject.getString("resultCode").equals(
				Constants.OPERATE_CODE_FAIL)) {
			resultinfo.put("resultCode", Constants.OPERATE_CODE_FAIL);
			resultinfo.put("resultMessage", jsonObject.getString("resultMsg"));
			return resultinfo;
		}
		SearchOneResponse searchOneResponse = new Gson().fromJson(result,
				SearchOneResponse.class);
	...
	public @ResponseBody Map<String, String> skipMessage(
			HttpServletRequest request, HttpServletResponse response)
			throws PaasException, IOException, URISyntaxException {
		...
		String result = HttpClientUtil.sendPostRequest(iPaasDubboUrl + "/ats/console/skipMessage", data);
	...
	public @ResponseBody Map<String, String> resendMessage(
			HttpServletRequest request, HttpServletResponse response)
			throws PaasException, IOException, URISyntaxException {
		Map<String, String> resultinfo = new HashMap<String, String>();
		UserInfoVo userInfoVo = UserUtil.getUserSession(request.getSession());
		...
		String data = JSonUtil.toJSon(params);
		String result = HttpClientUtil.sendPostRequest(iPaasDubboUrl + "/ats/console/resendMessage", data);
		JSONObject object = new JSONObject(result);
		resultinfo.put("resultCode", object.getString("resultCode"));
		resultinfo.put("resultMsg", object.getString("resultMsg"));
