        ...
        <div class="Open_cache_list"> 
          <div class="Open_cache_list_none">
			  <ul style="float:left;width:15%;text-align:right">
				<li><img src="${_base }/resources/images/huancun.png"></li>
			  </ul>
			  <ul style="float:left;width:55%;padding-left:3%;line-height:25px">
				<li style="font-size:16px;font-weight:600">简介</li>
				<li style="color:#949494">缓存服务（Memory Cache Service）是基于Redis技术实现，采用key-value数据结构，实现热点数据的高速缓存，可大幅提高应用的响应速度，极大缓解数据库访问的压力。MCS支持单例和集群模式，在集群模式下可实现高可用、高可靠、负载均衡。</li>
				<li>
					<a href="${_base}/mcs/toOpenCache"><div style="margin-top:20px;text-align:center;-moz-border-radius: 15px;border-radius: 15px;width:130px;height:30px;background:rgb(121,189,90);line-height:30px;vertical-align:middle;color:#fff">立即开通</div></a>
				</li>
			  </ul>
		 ...
		 <div class="tab_div">
		  ...
		  <div class="cache_pro_table"  style="table-layout:fixed;clear:both;">
			<table style="width:100%;height:100%;border-collapse:separate;border-spacing:10px 20px">
				<tr style="height:50%">
					<td>
						<div class="desc_title" style="margin:20px 10px 10px 10px;">热点数据访问提高应用响应速度</div>
						<div style="height:80px;margin:0px 10px;">实现热点数据的高速缓存，可大幅提高应用的响应速度，极大缓解数据库的访问压力。</div>
					</td>
					<td>
						<div class="desc_title" style="margin:20px 10px 10px 10px;">多类型数据结构</div>
						<div style="height:80px; margin:0px 10px;">支持Key-Value的数据结构，value可以是字符串(String), 哈希(Map), 列表(list), 集合(sets) 和 有序集合(sorted sets)等类型。</div>
					</td>
				</tr>
				<tr style="height:50%">
					<td>
						<div class="desc_title" style="margin:20px 10px 10px 10px;">服务可靠、安全保障</div>
						<div style="height:80px; margin:0px 10px;">当某台服务器宕机时，集群将迅速恢复服务，SDK会自动重连，服务恢复。<br/>3层账号密码验证，并可以限制服务器的IP地址，避免外部攻击。</div>
					</td>
					<td>
						<div class="desc_title" style="margin:20px 10px 10px 10px;">可伸缩</div>
						<div style="height:80px; margin:0px 10px;">当业务规模发生变化时，用户可随时根据需要修改云缓存实例的配置，并且在配置变更过程中，客户端程序也不必修改。</div>
					</td>
				</tr>
			</table>
		  </div>
		 ...
		 <div class="tab_div" id="tab_div2">
		  ...
		  <div class="cache_pro_help">
				<ul style="float:left;padding-left:20%;">
					...
					<li><a href="javascript:void(0);" onclick="javascript:downloadFile('PaaS-MCS.docx','2')">MCS使用说明书</a></li>
				</ul>
				<ul style="float:right;padding-left:10%">
					<li class="desc_title">常见问题</li>
					<li><a href="${_base }/help/FAQ#MCS1">多大的数据最适合存储在MCS上？</a></li>
					<li><a href="${_base }/help/FAQ#MCS2">MCS的数据支持持久化吗？</a></li>
				</ul>
		  </div>
		</div>  
		<!--3-->   
